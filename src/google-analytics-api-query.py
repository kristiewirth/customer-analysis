...
class GoogleAPI(object):
    ...
    def _get_service(self, api_name, api_version, scope, key_file_location):
        '''
        Default Google API code moved into a class.
    ...
    def intialize_API(self):
        '''
        Sets up API service and defines metrics and dimensions to be queried
        '''
        ...
        self.service = self._get_service('analytics', 'v3', scope, key_file_location)

        # All metrics
        self.metrics = ['ga:users', 'ga:newUsers', 'ga:percentNewSessions', 'ga:sessionsPerUser', 'ga:sessions', 'ga:bounces', 'ga:bounceRate', 'ga:sessionDuration',
        ...
        # All dimensions
        self.dimensions = ['ga:userType', 'ga:sessionCount', 'ga:daysSinceLastSession', 'ga:userDefinedValue', 'ga:userBucket', 'ga:sessionDurationBucket', 'ga:referralPath',
    ...
    def query_metrics(self):
        '''
        Queries all numerical variables using Google API
        '''
        ...
        overall_df.to_csv('../data/google-analytics/google_metrics_9-20-17.csv')

    def query_dimensions(self):
        '''
        Queries all categorical variables using Google API
        '''
        ...
        for drange in bar(self.dates):
            if one_date_range_df.empty:
                ...
                # Moved to creating individual files due to faster performance
                # overall_df = pd.concat([overall_df, one_date_range_df], axis=0)
                one_date_range_df.to_csv(
                    'google_dimensions_9-20-17_enddate_{}.csv'.format(drange[1]))
                one_date_range_df = pd.DataFrame()
            ...
            one_date_range_df.to_csv(
                '../data/google-analytics/google_dimensions_second_run_9-20-17_enddate_{}.csv'.format(drange[1]))
